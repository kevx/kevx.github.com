...
简而言之，在我们3台机器的测试集群上（标准JBOD方式SSD），4K Value长度下的TPS峰值可达200K以上，几乎可以秒杀其它任何一种分布式存储。
...
NVMe其实与AHCI一样都是逻辑设备接口标准，NVMe全称Non-Volatile Memory Express，非易失性存储器标准，是使用PCI-E通道的SSD一种规范，NVMe的设计之初就有充分利用到PCI-E SSD的低延时以及并行性，还有当代处理器、平台与应用的并行性。
